...
void real_AddNops( PBYTE start, PBYTE end )
{
	...
	while( iLeft )
	{
		if( AntiDebug.GenerateRandomNumber( 0, 7 ) < 5 )
		{
			nop_equ* p = possible_opcodes + AntiDebug.GenerateRandomNumber(0,ARRAYSIZE(possible_opcodes));
			if( !p->len ) continue;
			...
			int iSize = AntiDebug.GenerateRandomNumber( 0, iLeft );
			end[-(iLeft--)] = (BYTE)(iSize&0xFF);
			while( iSize ) {
				end[-iLeft] = (BYTE)(AntiDebug.GenerateRandomNumber(0,256)&0xFF);
				iSize--;
				iLeft--;
			}
		}
...
PBYTE __stdcall AllocateJumpgate( PBYTE target, int minlen, int maxlen )
{
	PBYTE buf;
	DWORD dwOld;
	int iLen = AntiDebug.GenerateRandomNumber( minlen, maxlen );
